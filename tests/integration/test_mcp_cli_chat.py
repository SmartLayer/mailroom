...
class ChatSession:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                prefix="mcp_chat_", suffix=".log", delete=False, mode="w"
            ) as temp:
            ...
            ):  # 30 seconds timeout for initialization
                if self.process.poll() is not None:
                    # Process exited
                    logger.error(
                        f"Chat process exited with code {self.process.returncode}"
                    )
    ...
    def send_command(self, command: str, timeout: Optional[int] = None) -> str:
        ...
        try:
            ...
            logger.info(
                f"Received response: {response[:500]}..."
...
class TestMcpCliChatMode:
    ...
    def test_natural_language_email_listing(self, chat_session):
        ...
        assert any(
            indicator in response.lower() for indicator in email_indicators
    ...
    def test_email_pagination(self, chat_session):
        ...
        assert any(
            indicator in first_page.lower() for indicator in email_indicators
        ), "First page doesn't appear to contain email information"
        assert any(
            indicator in second_page.lower() for indicator in email_indicators
    ...
    def test_direct_tool_call(self, chat_session):
        ...
        if json_matches:
            try:
                ...
                assert (
                    "pagination" in json_data
                ), "Missing 'pagination' field in JSON response"
