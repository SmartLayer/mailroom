...
def get_issue(issue_number: int) -> dict:
    ...
    result = run_gh_command(
        ["issue", "view", str(issue_number), "--json", "number,title,state,labels"]
    )
    return json.loads(result.stdout)


def update_issue_status(
    issue_number: int, status: IssueStatus, force: bool = False
) -> None:
...
def add_status_comment(issue_number: int, status: IssueStatus, message: str) -> None:
    ...
    run_gh_command(["issue", "comment", str(issue_number), "--body", comment_body])
...
def start_issue(issue_number: int) -> None:
    ...
    add_status_comment(
        issue_number, IssueStatus.IN_PROGRESS, "Work has started on this issue."
    )
...
def create_commit(
    ...
    try:
        subprocess.run(
            ["git", "commit", "-m", message], check=True, text=True, capture_output=True
        )
...
def create_pr(
    title: str, body: str, issue_number: Optional[int] = None, draft: bool = False
) -> None:
...
def complete_pr(
    pr_number: int, merge: bool = False, squash: bool = False, rebase: bool = False
) -> None:
    """Complete a PR by merging, squashing, or rebasing."""
    # Get PR information
    pr_result = run_gh_command(
        ["pr", "view", str(pr_number), "--json", "number,title,state,body"]
    )
    ...
    for issue_number in issue_refs:
        ...
        add_status_comment(
            issue_number, IssueStatus.COMPLETED, f"Issue completed with PR #{pr_number}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    start_parser = subparsers.add_parser(
        "start-issue", help="Start working on an issue"
    )
    ...
    commit_parser.add_argument(
        "--action",
        type=CommitAction,
        choices=list(CommitAction),
        default=CommitAction.REFS,
        help="Action to take in the commit",
    )
    ...
    merge_group.add_argument(
        "--squash", action="store_true", help="Squash and merge the PR"
    )
    merge_group.add_argument(
        "--rebase", action="store_true", help="Rebase and merge the PR"
    )
    ...
    status_parser.add_argument(
        "status", type=IssueStatus, choices=list(IssueStatus), help="New status"
    )
    status_parser.add_argument(
        "--force", action="store_true", help="Force status update even if already set"
    )
    status_parser.add_argument(
        "--comment", help="Comment to add explaining the status change"
    )
